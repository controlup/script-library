...
<#
    .SYNOPSIS
    Exits a Horizon machine (Linked/full clones) from maintenance mode

    .DESCRIPTION
    This script will issue the exit_maintenancemode command for a Horizon Linked or full Clone.
...
function exit-maintenancemode{
    ...
    try {
        $HVServices.machine.Machine_ExitMaintenanceMode($HVMachineID)
    }
    catch {
...
exit-maintenancemode -HVServices $objHVServices -HVMachineID ($objMachine.id)
