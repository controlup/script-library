...
<#
    ...
    Changelog:
        -   25-02-2020 - CHanged output and fixed errors when removing assignments from manual pools.
...
function remove-desktopassignment {
    param (
        [parameter(Mandatory = $true,
    ...
    try{
        # Assignments are removed by clearing out the user using the machineservice
        ...
        Out-CUConsole -Message 'There was a error removing the desktop assignment.' -Exception $_
    }
...
if ($HVPoolSpec.type -eq "AUTOMATED" -and $HVPoolSpec.AutomatedDesktopData.userassignment.userassignment -ne "DEDICATED") {
    Out-CUConsole -Message "This is not a dedicated desktop pool so can't remove assignments"
    exit
}
elseif ($HVPoolSpec.type -eq "MANUAL" -and $HVPoolSpec.ManualDesktopData.userassignment.userassignment -ne "DEDICATED" ) {
    ...
    remove-desktopassignment -HVConnectionServer $objHVConnectionServer -HVMachineID $HVMachineID
    out-cuconsole -Message "Desktop assignment succesfully removed."
}
