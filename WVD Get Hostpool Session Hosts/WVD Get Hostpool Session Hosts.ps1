...
.EXAMPLE
    Get-WVDHostPoolAllSessionHosts SessionHostName <SessionHostName>
.CONTEXT
        ...
        # Module is not loaded
        {
            ...
            # Module is not installed on the system
            {
                ...
                try
                {
                    ...
                    PowerShellGet\Install-Module -Name $($ModuleName) -Confirm:$false -Force -AllowClobber -Scope $psScope -SkipPublisherCheck -WarningAction SilentlyContinue
                }
                catch
                {
...
If ($azSPCredentials = Get-AzSPStoredCredentials)
{
    ...
    try 
    {
        ...
        # Ensure all Session Hosts are retrieved for the Hostpool
        $wvdSessionHostName = ""
    }
    catch 
    {
    ...
    Write-Host "WVD 2020 Spring Release Host Pool - Session Hosts: " -ForegroundColor Yellow
