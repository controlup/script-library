...
<#
    .SYNOPSIS
    This script will unpublish an App-V application for a user
...
if (Get-AppVClient -eq $true)
{
    ...
    try
    {
        ...
        if ($packages.count -ge 1)
        {
                ...
                {
                    If ($package.IsPublishedGlobally -eq $true)
                    {
                         $result = unpublish-AppvClientPackage -Name $package.Name
                         Write-host $package.Name 'has been unpublished for the current user'
                    }
                    else
                    {
                        Write-Output ($package.Name + ' is not currently published to this user')
                    }
