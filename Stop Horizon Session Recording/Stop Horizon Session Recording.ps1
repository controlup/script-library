...
<#
    .SYNOPSIS
    This scripts stops a recording of the Horizon Session Recording fling.
...
try{
    if((Get-HSRSessions -SessionID $sessionId).isrecording -eq $false){
        Out-CUConsole -message "No active recording found for $username on $computer"
        break
        }
}
Catch{
    Out-CUConsole -message "Error retreiving the active recording" -Exception $_
}
try{
    Out-CUConsole -Message "Stopping session recording for $username on $computer."
    stop-HSRSessionRecording -SessionID $sessionId
}
Catch{
    Out-CUConsole -Message "There was an error stopping the recording." -Stop
}
