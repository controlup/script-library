...
<#
    .SYNOPSIS
    This script will unpublish an App-V application
    ...
    try
    {
        ...
        if ($packages.count -ge 1)
        {
                ...
                foreach ($package in $packages)
                {
                    ...
                    {
                         $result = unpublish-AppvClientPackage -Name $package.Name -Global
                         Write-host $package.Name 'has been unpublished globally'
                    }
                    else
                    {
                        Write-Output ($package.Name + ' is not currently published globally')
                    }
