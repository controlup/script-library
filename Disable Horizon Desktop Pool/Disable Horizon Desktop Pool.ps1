...
<#
    ...
    .DESCRIPTION
    This script disables a Horizon Desktop pool through the VMware.Hv.Helper module

    .EXAMPLE
    Can be used as an Automated action to disable a Horizonl Desktop pool for plannend maintenance
...
function Set-HorizonViewPoolEnablement {
    param (    
        [parameter(Mandatory = $true,
    ...
    if ($Enable) {
        try {
            Set-HVPool -Pool $HVPool -HvServer $HVConnectionServer -Enable
            Out-CUConsole -Message 'Pool enabled.'
        }
        catch {
            Out-CUConsole -Message 'There was a problem enabling the Horizon View pool.' -Exception $_
        }
    }
    else {
        try {
            Set-HVPool -Pool $HVPool -HvServer $HVConnectionServer -Disable
            Out-CUConsole -Message 'Pool disabled.'
        }
        catch {
            Out-CUConsole -Message 'There was a problem disabling the Horizon View pool.' -Exception $_
        }
    }
}
...
# Disable the Horizon Virtual Desktop Pool
Set-HorizonViewPoolEnablement -HVPool $objHVPool -HVConnectionServer $objHvConnectionServer -Enable:$false
