...
<#
    ...
    .DESCRIPTION
    This script will migrate the VM to a new host.
    ...
    .PARAMETER srtVMWareHost
    The name of the host the VM has to be migrated to (preferably FQDN)
...
function Migrate-VMWareVMMachine {
    param (
        [parameter(Mandatory = $true)]
        $objVM,
        [parameter(Mandatory = $true)]
        $objVCenter,
        [parameter(Mandatory = $true)]
        [string]$strTargetVIContainer
    )

    # Check if target host is different than current host.
    If ($objVM.VMHost.Name -eq $strTargetVIContainer) {
        Feedback -Message "VM could not be moved to host $strTargetVIContainer because the VM is already hosted on it." -Oops
    }

    try {
        Move-VM -VM $objVM -Server $objVCenter -Confirm:$false -Destination $strTargetVIContainer
        # Target VI Container is folder, host, cluster, or a resource pool 
    }
    catch {
        Feedback -Message 'The VM could not be migrated.' -Exception $_
    }
}
...
# Move the VM
Migrate-VMWareVMMachine $objVM $objVCenter $strVMWareHost | Select-Object -Property VMHost | Format-List

# Send feedback
Feedback -Message 'The VM is being migrated.'
