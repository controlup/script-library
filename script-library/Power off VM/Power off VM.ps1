...
<#
    .SYNOPSIS
    Power off the VM hardware

    .DESCRIPTION
    This script will power off the VM (virtual) hardware.
...
function PowerOff-VMwareVMHardware {
    param (
        [parameter(Mandatory = $true)]
        $objVM,
        [parameter(Mandatory = $true)]
    ...
    try {
        Stop-VM -VM $objVM -Server $objVCenter -Confirm:$false
    }
    catch {
        Feedback -Message 'The VM could not be powered off.' -Oops
    }
}
...
# PowerOff VM hardware
PowerOff-VMwareVMHardware $objVM $objVCenter | Select-Object PowerState | Format-List
