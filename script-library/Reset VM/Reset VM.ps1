...
<#
    .SYNOPSIS
    Resets VM on hardware level

    .DESCRIPTION
    This script resets the VM machine {virtual) hardware.
...
function Reset-VMWareVMHardware {
    param (
        [parameter(Mandatory = $true)]
        $objVM,
        [parameter(Mandatory = $true)]
    ...
    try {
        Restart-VM -VM $objVM -Server $objVCenter -Confirm:$false | Out-Null
    }
    catch {
...
# Reset VM hardware
Reset-VMWareVMHardware $objVM $objVCenter

# Send confirmation
Feedback -Message 'The VM hardware has been sent the reset command.'
