<#
    ...
    .EXAMPLE
        . .\IGEL_UpdatePolicy.ps1 -device IGEL01 -Command Reboot -UMSServer igelums.acme.local
        Executes an IGEL Device Reboot Command

    .EXAMPLE
         . .\IGEL_UpdatePolicy.ps1 -device IGEL01 -Command Shutdown -UMSServer igelums.acme.local:9943
        Executes an IGEL Device Shutdown Command

    .EXAMPLE
         . .\IGEL_UpdatePolicy.ps1 -device IGEL01 -Command Wakeup -UMSServer igelums.acme.local:9943
        Executes an IGEL Device WakeUp command against the device

    .EXAMPLE
         . .\IGEL_UpdatePolicy.ps1 -device IGEL01 -Command Update -UMSServer igelums.acme.local:9943
        Sync's the settings from the UMS server to the IGEL endpoint
    ...
    .MODIFICATION_HISTORY
        ...
        Modified Marcel Calef : 2020-02-10
...
Param (
    ...
    [Parameter(Mandatory=$true,HelpMessage='Command to execute')][ValidateSet('Reboot','Shutdown','Wakeup','Update','Details','Profile','Asset')]   [string]$Command
...
switch ($command) {
    ...
    "Asset"     { $commandResult = Invoke-IGELRestAPI -URI "https://$UMSServer/umsapi/v3/assetinfo/thinclients/$($tcId)"               -headers $headers -method GET }
    "Profile"   { 
...
if ([bool]($commandResult.PSobject.Properties.name -match "Content")) {
    ...
    if ($commandResult.Content -like "*CommandExecList*") {
        ...
        #Write-Output "$(($commandResult.Content | ConvertFrom-Json) |Out-String)"
            $($commandResult.Content | convertfrom-json).assetinfos | select "assetId","connector","assetName","assetVendor","deviceId" | format-table
    }
} else {
    #Write-Output "$($commandResult | Out-String)"
   # Write-Output "$($commandResult.Content | convertfrom-json).assetinfos"
}
